# 📉 1.2.2 Falta de espaço em disco
**Categoria:** Infraestrutura  
**CI afetado:** Volume Data / Volume Log / TempDB  
**Escopo:** Arquivos MDF / NDF / LDF  
**Severidade sugerida:** P1 / P2  

---

## 🎯 Objetivo

Identificar consumo excessivo de espaço em disco que possa comprometer operações do SQL Server, incluindo crescimento de datafiles, transaction log e TempDB.

---

## 🔎 Diagnóstico Inicial (Quick Triage)

Ferramenta opcional:

- EXEC sp_whoisactive;
```sql
SELECT name, size, max_size, growth
FROM sys.database_files;
DBCC SQLPERF(LOGSPACE);
```

---

🧠 **Análise por DMV**
- **Arquivos do Banco de Dados**
```sql
SELECT 
    name,
    size * 8 / 1024 AS SizeMB,
    max_size,
    growth
FROM sys.database_files;
```
Objetivo:

- Verificar crescimento e limites dos arquivos

##

- **Consumo de Transaction Log**
- DBCC SQLPERF(LOGSPACE);

Objetivo:

- Identificar bancos com log próximo do limite

##

- **TempDB**
```sql
SELECT *
FROM tempdb.sys.database_files;
```
Objetivo:

- Validar crescimento inesperado da TempDB

---

🖥️ **PerfMon (Sistema Operacional)**

Principais contadores:

-LogicalDisk% Free Space
-LogicalDisk\Free Megabytes

Objetivo:

- Confirmar nível crítico de espaço disponível

---

🧰 **ScriptDBA / Ferramentas Opcionais**

Ferramenta não nativa:

- sp_whoisactive

Uso recomendado para identificar sessões consumindo espaço excessivo.

---

🧩 **Possíveis Causas**

- Crescimento descontrolado de transaction log
- Falta de rotina de backup de log
- Índices grandes ou fragmentados
- Queries com uso intenso de TempDB
- Arquivos configurados com autogrowth inadequado

---

🛠️ **Ações de Mitigação**

- Realizar backup do transaction log
- Avaliar limpeza de dados históricos
- Ajustar tamanho inicial dos arquivos
- Revisar estratégia de autogrowth
- Verificar consultas que utilizam TempDB intensivamente

---

🔁 **Monitoramento Pós-Mitigation**
```sql
SELECT name, size * 8 / 1024 AS SizeMB
FROM sys.database_files;
```
```sql
DBCC SQLPERF(LOGSPACE);
```
